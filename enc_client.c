...
char* readF(char* fName){
	...
	//receive txt from file
	len=getline(&text, &size, file);

	//if any char is not a space or not in range of 65 to 90, txt file is bad
...
int main(int argc, char *argv[]) {
	...
	//check lengths of string and key
	kLen=strlen(key);
	pLen=strlen(plain);
	if(pLen>kLen)
		error("Client: ERROR key length is too short");


	//send pLen
	charsWritten = send(socketFD, &pLen, sizeof(pLen), 0); 
	if (charsWritten < 0)
	...
	//send plaintxt
	charsWritten = send(socketFD, plain, strlen(plain), 0); 
	if (charsWritten < 0)
		error("CLIENT: ERROR writing plain to socket");
	
	if (charsWritten < strlen(plain))
	...
	cipher=malloc(pLen+1);
	charsRead = recv(socketFD, cipher, strlen(plain)+1, 0); 
	if (charsRead < 0)
